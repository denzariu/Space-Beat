...
public class Joycon
{
    ...
    public Quaternion GetVector()
    {
        ...
        // Y needs to be Z
        //return Quaternion.LookRotation(new Vector3(j_b.x, i_b.x, k_b.x), new Vector3(j_b.y, i_b.y, k_b.y));

        //1
        //return Quaternion.LookRotation(new Vector3(j_b.x, i_b.x, k_b.x), new Vector3(j_b.z, i_b.z, k_b.z));

        //2
        //return Quaternion.LookRotation(new Vector3(j_b.y, i_b.y, k_b.y), new Vector3(j_b.x, i_b.x, k_b.x));
        
        //3
        return Quaternion.LookRotation(new Vector3(j_b.z, i_b.z, k_b.z), new Vector3(j_b.x, i_b.x, k_b.x));
    ...
    public void Update()
    {
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                DebugPrint(string.Format("Dequeue. Queue length: . Packet ID: . Timestamp: . Lag to dequeue: . Lag between packets (expect 15ms): "), DebugType.THREADING);
                ts_prev = rep.GetTime();
            }
