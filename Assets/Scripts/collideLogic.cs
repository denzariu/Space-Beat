...
using UnityEngine;
using Assets.Scripts.Slice_Scripts;
using MilkShake;
...
public class collideLogic : MonoBehaviour
{
    ...
    private float _forceAppliedToCut = 12f;
    ...
    public JoyconDemo joyconScript;
    ...
    void Start()
    {
        ...
        joyconScript = parentGO.transform.parent.gameObject.GetComponent<JoyconDemo>();
        joycon = joyconScript.getJoyCon();

        audioPlayer = _tip.GetComponent<AudioSource>();

        //if (!joycon.isLeft) { joycon.SetRumble(160, 320, 0.6f, 200); }
...
/*
        ...
        catch { _vertices[_frameCount] = _previousBasePosition; }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.gameObject.tag == "Target")
        {
            ...
            // Check whether projectile was bad or good
            if (other.gameObject.GetComponent<despawnBehaviour>().itHurt())
            {
                // Massive Camera Shake
                GM.ShakeCameraHard();

                // Activate JoyCon Rumble
                try { joycon.SetRumble(180, 320, 0.5f, 150); }
                catch
                {
                    //print("Joystick couldn't rumble.");
                }
            }
            else
                // Activate JoyCon Rumble
                try { joycon.SetRumble(300, 320, 0.3f, 150); }
                catch
                {
                    //print("Joystick couldn't rumble.");
                }

            // Initiate object despawn
            other.gameObject.GetComponent<despawnBehaviour>().Hit();

            if (other.gameObject.GetComponent<despawnBehaviour>().isDead())
            {
                ...
                GameObject[] slices = Slicer.Slice(plane, other.gameObject, other.GetComponent<Rigidbody>().mass);

                other.gameObject.GetComponent<despawnBehaviour>().Die();

                Vector3 dir = new Vector3(0, 0, 0);

                if (transformedNormal.z > 0.35)
                    dir = Vector3.left;
                else if (transformedNormal.z < -0.35)
                    dir = Vector3.right;
                else if (transformedNormal.x < 0.45)
                    dir = Vector3.forward;
                else if (transformedNormal.x > 0.45)
                    dir = Vector3.back;
                else dir = Vector3.one;
                ...
                //rigidbody.AddForce(transformedNormal + _forceAppliedToCut * -dir, ForceMode.Impulse);
                //rigidbody.AddForce(new Vector3(-transformedNormal.z, -transformedNormal.y, -transformedNormal.x) * _forceAppliedToCut, ForceMode.Impulse);
                rigidbody.AddForce(-transformedNormal * _forceAppliedToCut, ForceMode.Impulse);

                Rigidbody rigidbody2 = slices[0].GetComponent<Rigidbody>();
                //newNormal = transformedNormal + Vector3.down * _forceAppliedToCut;
                //newNormal = -newNormal;
                //rigidbody2.AddForce(newNormal, ForceMode.Impulse);


                //rigidbody2.AddForce(transformedNormal + _forceAppliedToCut * dir, ForceMode.Impulse);
                //rigidbody2.AddForce(-new Vector3(-transformedNormal.z, -transformedNormal.y, -transformedNormal.x) * _forceAppliedToCut, ForceMode.Impulse);
                rigidbody2.AddForce(transformedNormal * _forceAppliedToCut, ForceMode.Impulse);

                Destroy(slices[0], timeToDespawn);
                Destroy(slices[1], timeToDespawn);
            }
        }
